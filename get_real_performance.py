...
# 使用实际数据与diff天前的预测数据进行比对，得出模型真实性能，存入数据库中
def get_real_performance(method_type, threshold, diff):
    conn = pymysql.connect(host='10.214.163.179', user='dt_yc', password='dt_yc123', port=3306, database='dt_yc')
    cursor = conn.cursor()

    current_date = get_current_date()
    previous_date = get_previous_diff_date(current_date, diff)

    # 从数据库中得到previous_date到current_date时间段内发生故障电梯的tranid集合
    query = """
            SELECT tranid FROM dt_yc.zt_dt_fault
            WHERE form_create_time >= DATE_FORMAT(%s, '%%Y-%%m-%%d')
            AND form_create_time <= DATE_FORMAT(%s, '%%Y-%%m-%%d')
            """
    val = [previous_date, current_date]
    ...
    # 从csv中读取previous_date的预测结果
    csv_file = open('results/' + str(previous_date) + '-result.csv', 'r')
    ...
    val = [method_type, previous_date, precision, recall, f1_score]
